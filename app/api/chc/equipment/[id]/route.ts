...
function parseEquipmentType(value: unknown): EquipmentType | null {
  ...
  return Object.values(EquipmentType).includes(normalized as EquipmentType) ? normalized as EquipmentType : null;
}
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const equipment = await prisma.equipment.findFirst({ where: { id, chcId: user.profileId } });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!name || !type) return NextResponse.json({ error: "Equipment name and type are required" }, { status: 400 });
    if (purchaseYear !== null && (!Number.isInteger(purchaseYear) || purchaseYear < 1900 || purchaseYear > new Date().getFullYear())) return NextResponse.json({ error: "Enter a valid purchase year" }, { status: 400 });

    const existing = await prisma.equipment.findFirst({ where: { id, chcId: user.profileId }, select: { id: true } });
    if (!existing) return NextResponse.json({ error: "Equipment not found" }, { status: 404 });

    const equipment = await prisma.equipment.update({ where: { id }, data: { name, type, brand, model, registrationNumber, purchaseYear } });
    return NextResponse.json({ equipment });
  } catch (error: unknown) {
    const errorCode = error && typeof error === "object" && "code" in error ? error.code : undefined;
    if (errorCode === "P2002") return NextResponse.json({ error: "That registration number is already in use" }, { status: 409 });
