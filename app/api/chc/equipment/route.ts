...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (errorCode === "P2002") {
      return NextResponse.json({ error: "That registration number is already in use" }, { status: 409 });
    }
    console.error("Equipment creation error:", error);
    return NextResponse.json({ error: "Unable to create equipment" }, { status: 500 });
  }
}