...
function parsePricingUnit(value: unknown): PricingUnit | null {
  ...
  if (Object.values(PricingUnit).includes(normalized as PricingUnit)) return normalized as PricingUnit;
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!Number.isFinite(price) || price <= 0 || !pricingUnit) {
      return NextResponse.json({ error: "A valid price and pricing unit are required" }, { status: 400 });
    }

    const existing = await prisma.cHCService.findFirst({ where: { id, chcId: user.profileId } });
    if (!existing) return NextResponse.json({ error: "Service offer not found" }, { status: 404 });

    const service = await prisma.cHCService.update({ where: { id }, data: { price, pricingUnit } });
    return NextResponse.json({ service });
  } catch (error: unknown) {
    console.error("CHC service update error:", error);
    return NextResponse.json({ error: "Unable to update service offer" }, { status: 500 });
  }
}
