...
export default function AddEquipment() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/chc/equipment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to create equipment");
      router.push("/dashboard/chc/equipment");
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Unable to create equipment");
      setSaving(false);
    }
  };
