...
export default function EquipmentListing() {
  ...
  useEffect(() => {
    fetch("/api/chc/equipment").then(async (response) => {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 sm:px-10">
      ...
      <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{equipment.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div className="rounded-xl bg-slate-100 p-3 text-slate-700"><Wrench className="h-5 w-5" /></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold capitalize text-emerald-700">{item.status}</span></div><h3 className="mt-5 text-lg font-bold">{item.name}</h3><p className="mt-1 text-sm capitalize text-slate-500">{item.type}{item.brand ? ` · ${item.brand}` : ""}{item.model ? ` ${item.model}` : ""}</p>{item.registrationNumber && <p className="mt-5 border-t border-slate-100 pt-4 text-xs font-semibold uppercase tracking-wider text-slate-400">Reg. {item.registrationNumber}</p>}</article>)}</div>
      {!error && equipment.length === 0 && <div className="mt-10 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><Wrench className="mx-auto h-10 w-10 text-slate-300" /><h3 className="mt-4 text-lg font-bold">No equipment yet</h3><p className="mt-2 text-sm text-slate-500">Add your first machine to unlock matching services.</p></div>}
    </div>
  );
}