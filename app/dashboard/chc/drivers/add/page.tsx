...
const disabledInputClass = "mt-2 h-12 w-full rounded-xl border border-slate-200 bg-slate-100 px-4 font-normal text-slate-400 cursor-not-allowed outline-none select-none";
...
export default function AddDriverPage() {
    ...
    const submit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await fetch("/api/chc/drivers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || "Unable to add driver");
            router.push("/dashboard/chc/drivers");
        } catch (err: unknown) {
            setError(err instanceof Error ? err.message : "Unable to add driver");
            setSaving(false);
        }
    };
