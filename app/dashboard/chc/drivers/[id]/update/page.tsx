...
export default function UpdateDriverPage() {
    ...
    const submit = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); setError(""); try { const response = await fetch(`/api/chc/drivers/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Unable to update driver"); router.refresh(); router.push("/dashboard/chc/drivers"); } catch (err: unknown) { setError(err instanceof Error ? err.message : "Unable to update driver"); setSaving(false); } };
